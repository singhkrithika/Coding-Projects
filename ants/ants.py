...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(ant_in_place):
                place.ant = self
                self.store_ant(ant_in_place)
            elif ant_in_place.can_contain(self):
                ant_in_place.store_ant(self)
                self.ant = ant_in_place
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current.is_hive == False:
            if current != None and current.bees != []:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current != None:
            if current.ant != None:
                current.ant.double()
                if current.ant.is_container == True and current.ant.ant_contained != None:
