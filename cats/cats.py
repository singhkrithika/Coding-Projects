...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for x in word_list:
        if diff > diff_function(typed_word, x, limit):
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[x[i+1] - x[i] for i in range(len(x)-1)] for x in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    final = [[] for x in player_indices]

    for x in word_indices:
        best_player = 0
        best_time = get_time(times,0, x)
        for y in player_indices:
            if get_time(times,y, x) < best_time:
                best_time = get_time(times,y, x)
                best_player = y
        final[best_player] = final[best_player] + [words[x]]
    return final        
