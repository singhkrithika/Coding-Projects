...
def boar_brawl(player_score, opponent_score):
    ...
    result = 3 * abs(tens_digit - ones_digit)
    if 1 > result:
        result = 1
    return result
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    is_sus = (num_factors(score) == 3 or num_factors(score) == 4)
    if is_sus:
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def player_strategy(current_score, opponent_score):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    num_rolled = strategy(0,0)
    n = 0
    while (n < goal):
        k = 0
        while (k < goal):
            if num_rolled != strategy(n,k):
                return False
            k += 1
        n += 1
    return True
